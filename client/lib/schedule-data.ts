...
export const SCHEDULE_GAMES: ScheduleGame[] = [
  {
    ...
    keys: [
      ...
      "Gap integrity vs explosive runs",
    ],
    swing: [
      { name: "Aaron Philo", role: "QB1 — protect the ball vs FAU pressure looks" },
      { name: "Jayden Woods", role: "JACK — set the edge and rush the passer" },
      { name: "Eric Singleton Jr.", role: "WR — stress soft coverage when FAU loads the box" },
    ],
    film:
      "Sources: ESPN highlight packages — FAU W at Rice 27-21 (Oct 4, 2025) and W vs Tulsa 40-21 (Nov 8, 2025). Shotgun Air Raid, pass-heavy, vertical shots + explosive runs confirmed. Tempo/RPO not confirmed on these cuts.",
    pred: "UF 38 · FAU 10",
    predUF: 38,
    predOpp: 10,
    ...
    howUFWins: [
      "Veltkamp was efficient in rhythm when protected (24/33 for 290 vs Rice; 22/29 for 272 vs Tulsa on ESPN highlights) — pressure before the first read.",
      "Confirmed vertical score vs Rice (~46-yard TD clip) — safety help over the top; do not play soft boundaries.",
      "Confirmed long TD run vs Rice (~68 yards) — maintain gap fits; do not free-hit.",
    ],
    scoutingReport:
      "From 2025 ESPN highlight packages of FAU's wins at Rice (27-21) and vs Tulsa (40-21): FAU lives in shotgun spread under Zach Kittley. QB Caden Veltkamp is efficient when protected and will take vertical shots (confirmed deep TD vs Rice). They can also hit explosive runs when blocking holds (confirmed long TD run vs Rice). Highlight cuts do not show between-snap tempo or clear RPO — those are not claimed here. UF keys from this tape: pressure to break rhythm, help over the top on verticals, gap integrity vs the run.",
    tickets: {
