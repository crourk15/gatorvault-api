import seedJson from './game-zone-hub-seed.json';
import type { BettingGame } from './betting-api';

export type GameZoneHubSeed = {
  generatedAt: string;
  source: string;
  nextGame: BettingGame | null;
  lastGame?: BettingGame | null;
  finals?: Record<string, { uf: number; opp: number; source?: string }>;
};

export const GAME_ZONE_HUB_SEED = seedJson as unknown as GameZoneHubSeed;

export function buildSeedNextGame(): BettingGame | null {
  return GAME_ZONE_HUB_SEED.nextGame || null;
}

export function buildSeedLastGame(): BettingGame | null {
  return GAME_ZONE_HUB_SEED.lastGame || null;
}
