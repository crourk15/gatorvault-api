...
import { RECRUITING_HUB_HERO_SEED, recruitingHubHeroSeedForYear } from '@/lib/recruiting-hub-hero-seed';

const CLASS_YEARS = [2026, 2027, 2028] as const;

/** Server-rendered hero — real class metrics baked in for first paint. */
export function RecruitingHubHeroSsr({
  year = RECRUITING_HUB_HERO_YEAR,
}: {
  year?: number;
}): React.ReactElement {
  const overview = recruitingHubHeroSeedForYear(year);
  const title = RECRUITING_HUB_HERO_SEED.title;
  const subtitle = RECRUITING_HUB_HERO_SEED.subtitle;

  return (
    <>
      <section
        className="rh-hero-strip"
        data-hydrate="hero"
        data-hero-seeded="true"
      ...
      >
        ...
        <div className="rh-hero-metrics" aria-label="Class summary metrics">
          ...
          <div className="rh-hero-metric">
            <span className="rh-hero-metric__label">{overview.commitLabel || 'Commits'}</span>
