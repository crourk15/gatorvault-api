...
function movementText(movement: PredictionIntel['movement'], ufPct: number): string {
  if (movement === 'up') return `Movement this week: +${Math.max(1, Math.round(ufPct / 30))}%`;
  if (movement === 'down') return `Movement this week: -${Math.max(1, Math.round((100 - ufPct) / 30))}%`;
  return 'Movement this week: flat';
}
    ...
    <div className="gv-gw-wp-gauge" data-testid="gw-wp-gauge">
      ...
      <div className="gv-gw-wp-gauge__rings">
        <svg className="gv-gw-wp-gauge__svg" viewBox="0 0 128 128" aria-hidden="true">
          ...
          />
          <text x="64" y="60" textAnchor="middle" className="gv-gw-wp-gauge__pct">
            {pct}%
          </text>
          <text x="64" y="76" textAnchor="middle" className="gv-gw-wp-gauge__uf-label">
            UF
          </text>
        </svg>
      </div>
      ...
      <span className="gv-gw-wp-gauge__confidence">
