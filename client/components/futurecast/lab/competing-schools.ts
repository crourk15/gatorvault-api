...
export function isFlorida(name: string): boolean {
  const t = String(name || '');
...
export function shortSchoolLabel(name: string): string {
  const n = normalizeSchool(name);
  // Order matters — more specific names before shorter/ambiguous ones.
  ...
  return words[0]?.slice(0, 10) || n.slice(0, 10) || 'Peer';
}
