#!/usr/bin/env node
/**
 * Create App Store Connect iOS version 1.0.3, attach build 14, fill What's New,
...
const SHOT_PATH = path.join(__dirname, '..', 'docs', 'app-store', 'asc-1-0-3-submit.png');
...
async function attachBuild14(page) {
  log('Looking for Build selector / build 14...');
  await dismissOverlays(page);

  const bodyBefore = await page.locator('body').innerText().catch(() => '');
  if (/BUILD[\s\S]{0,120}14[\s\S]{0,40}1\.0\.3/i.test(bodyBefore)) {
    log('Build 14 already attached — skipping picker (avoids drop-holder overlay).');
  ...
  let picked = false;
  const candidates = [
    page.locator('[role="dialog"] tr:has-text("14")'),
    page.locator('.drop-holder tr:has-text("14"), .holder-el tr:has-text("14")'),
    page.locator('[role="row"]:has-text("14")'),
  ];
  for (const c of candidates) {
    ...
    try {
      ...
      picked = true;
      log('Clicked build 14 candidate.');
      break;
    } catch (_) {}
  }
...
async function submitForReview(page) {
  ...
  try {
    ...
    await sleep(4000);
    const draftRow = page.locator('tr:has-text("1.0.3"), [role="row"]:has-text("1.0.3")').first();
    if (await draftRow.count()) await draftRow.click().catch(() => {});
    else await page.getByText(/iOS 1\.0\.3|Today at/i).first().click().catch(() => {});
    await sleep(2500);
...
async function main() {
  ...
  try {
    ...
    // Go straight to known inflight 1.0.3 page
    ...
    const inflightText = await waitForAscContent(
      page,
      /1\.0\.3|Build|What.?s New|Prepare for Submission|Add for Review/i,
      90000
    );
    if (!/1\.0\.3/i.test(inflightText)) {
      throw new Error('Inflight page did not show 1.0.3. UI: ' + inflightText.slice(0, 400));
    }
    log('Opened 1.0.3 inflight page.');

    const buildResult = await attachBuild14(page);
...
main().catch((err) => {
  console.error('[asc-1.0.3] FATAL:', err.message || err);
  process.exit(1);
});