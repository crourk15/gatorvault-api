/**
 * FutureCast Elite API Response Types
 *
...
/** Four core Elite card metrics returned by FutureCast board APIs. */
export interface FutureCastEliteCoreMetrics {
  /** UF % (Likelihood) — also exposed as `ufConfidence` on allow-list board payloads. */
  ufProbability: number;
  /** Staff % (Insider Confidence). */
  staffConfidence: number;
  /** Fit % (Scheme Match). */
  fitScore: number;
  /** Priority Score (Importance) — not a commit probability. */
  priorityScore: number;
}

/** Shared identity and ranking fields on Elite player responses. */
...
/** Base Elite player shape combining core metrics + shared fields. */
