...
const VERB_RE =
  /\b(is|are|was|were|has|have|had|put|made|says|said|lists|listed|keeps|kept|earned|moved|told|cracked|spends|spending|remains|remained|became|gives|gave|helps|helped|shows|showed|landed|texting|visited|visiting|building|climbing|liked|knows|know|didn't|doesn't|could|can|will|would|should|makes|remains|visited|watched|wants|want|called|calls|opened|opens|pushed|pushes|gave|gives|impressed|impresses|firmly|firm|holds|hold|surprised|surprises|separated|separates|separation|picked|picks|spending|spent|gained|gain|gaining|moved|moving|need|needs|fit|fits|stay|stays|pull|pulls|remains|remain|open|opened|separated|responded|respond|leaned|lean|leaning|widened|widen|positioned|position|working|work|calling|call|separate|separates|left|leave|pressing|press)\b/i;

const FRAGMENT_RE =
  /(?:^|[\s(])(?:visit|board|staff|spring|campus|gainesville|swamp|top schools|leaderboard)\s*\+|^[^.!?]*\+\s*[^.!?]*\.?$|^[^.!?]*\bwith on\.\s*$|^[^.!?]*\bwith first visit to\.\s*$|^[^.!?]*\b—\s*[a-z]|\b(?!(?:After|Before|During|Once)\b)[A-Z][a-z]+\s+his\s+on-campus\b/i;
