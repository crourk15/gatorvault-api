...
const FRESH_OFFER_RE =
  /\b(?:today|this\s+(?:morning|afternoon|evening|week|weekend)|just\s+(?:offered|got|received)|new\s+offer|re-?offered|sources?\s+confirm(?:s|ed)?\s+(?:the\s+)?offer|broke\s+the\s+news|per\s+(?:the\s+)?(?:beat|report).*offer)\b/i;

const NARRATIVE_PIVOT_RE =
  /\b(?:months?\s+later|now\s+says|says\s+the\s+gators|told\s+(?:reporters|us)|said\s+(?:that|the)|cementing|resonating|build(?:ing)?\s+trust|major\s+contender|relationship\s+with)\b/i;
