 ...
 * Strict / Private Key Mode: send user_id (public key) + accessToken (private key).
...
function emailJsErrorHint(status, text) {
  ...
  if (status === 403 && body.includes('strict mode')) {
    return 'Private Key Mode requires accessToken — set EMAILJS_PRIVATE_KEY and enable Use Private Key in EmailJS Security';
  }
