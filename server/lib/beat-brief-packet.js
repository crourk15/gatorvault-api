...
function formatProjectionCompBlock(research = null, player = null) {
  ...
  const lines = [];
  lines.push('PLAYER PROJECTION / COMP (ALWAYS required in recruit posts)');
  lines.push('----------------------------------------------------------------');
...
function formatFilmTraitsBlock(filmTraits) {
  ...
  lines.push('-------------------------------------------------------------');
  lines.push(
    'AGENT STEP (do this before drafting): Open every highlight URL below and actually review the tape (not the title). Write a DEEPER film eval into the post — 2–3 concrete observations with specificity (platform/drop, release/arm slot, eyes vs pressure, sideline vs intermediate window, red-zone look, length over the line). Avoid generic “good arm / athletic” filler. Do not invent tackle totals, completion %, or grades. Do not tell the audience you watched film — just use what you saw as Vault fact.'
  );
...
function formatBriefText({
  ...
  lines.push(
    'HEADER RULE (hard): First line of your deliverable must be `HEADER: …` — an elite intel leap, not a board sticker. Match the ENERGY of top recruiting hooks (urgent, specific, Florida-relevant) but write ORIGINAL Vault copy — never reuse “New intel has emerged…” or any beat-writer phrasing. Forbidden: cloned openers, “owns/own,” flat “Florida leads [Player]” labels, “Breaking,” emoji spam, Hudl, “soph Hudl,” highlight reel, “I watched the film,” “the tape we sat.” Fans never hear the sit source. Then a blank line, then the post body. Body opener should not just repeat the header.'
  );
  lines.push(
    'FILM REVIEW RULE (hard): Before writing, open every Highlight LINK in FILM / HIGHLIGHTS. Review the Hudl/On3 tape in depth. Your post must include 2–3 concrete, specific observations from that tape (platform/drop, release, eyes vs pressure, intermediate/sideline windows, red-zone look, length over the line). No generic “good arm.” If a link will not open, write from board facts only — do not invent film. FAN COPY RULE (hard): Never tell fans you watched Hudl, On3, a highlight reel, or “the tape we sat.” Forbidden in HEADER, body, and fan Vault Scouting: Hudl, “soph Hudl,” highlight reel, “on the tape we have,” “no Hudl sit,” “I watched the film.” State tape facts as Vault fact. Staff notes / persist JSON may name Hudl.'
  );
  lines.push(
    'PROJECTION / COMP RULE (hard): ALWAYS include both in the post body (mid-body, after identity/tape). Prefer PLAYER PROJECTION / COMP values when on file (still verify size). If marked “(none on file)”, YOU MUST draft both from board + tape: (1) forward projection = contribution timeline + role + ceiling matched to stars/rank band AND position reality; (2) one GatorVault player comp. QB PROJECTION RULE: never write “rotational QB” / generic “Year 2–3 rotation / every-down” filler — project develop → compete for the job → starter upside (or campus QB battle). Other positions may use rotation/every-down when it fits. COMP SIZE RULE (non-negotiable): listed height within ~1–2 inches of the recruit; same frame/build family; then match win traits from tape. A traits-only match at the wrong height is a FAIL (e.g. 6-5 / 190 recruit cannot comp to a 6-1 pocket QB). State the size logic briefly when helpful (“6-5 mold…”). Never a higher elite household name than the band. Do NOT default to Florida alumni. Never omit either. Never close on the comp.'
  );
  lines.push(
    'VOICE RULE (hard): This is GatorVault\'s take — not a beat recap. Absorb facts from the intel seed/board + what you saw on the highlight link, then rewrite in original Vault voice. FORBIDDEN in the post: naming beat writers; "beat line"; "according to reports"; "per On3/247"; "as reported"; "reports say"; "insiders say"; "I watched the film"; Hudl; "soph Hudl"; highlight reel; "on the tape we have"; "no Hudl sit"; and any meta flex about being different or ahead of the timeline. Just state the board + film. Board facts and tape observations may be stated as Vault fact.'
  );
  lines.push(
  ...
  lines.push(
    'PERSIST RULE (hard): After you draft eval + GatorVault comp + projection for a 2028 recruit/target, you MUST write them into War Room (and film-traits) so FutureCast / player profiles pick them up. Do not leave comps/projections only in the X draft. Use: `node server/scripts/upsert-vault-film-eval.js --file=server/data/war-room/vault-evals/<slug>.json` (or pass --slug/--comparison/--projection/--strengths/--hudl). Fan fields (`insiderNotes`, `angle` / `vaultFilmAngle`) follow FAN COPY RULE — never name Hudl there. Staff notes / source URLs may. Then confirm the Copy Brief Projection/Comp card is no longer empty on next Open.'
  );
...
async function buildRosterDeskBrief(slug, roster, opts = {}) {
  ...
  lines.push(
    ...
    'VOICE RULE (hard): GatorVault original take. FORBIDDEN: naming beat writers, "according to", "reports say", "per On3/247", "I watched the film", Hudl, "soph Hudl", highlight reel.',
