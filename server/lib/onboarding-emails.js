...
function activateBodyHtml({ name, email } = {}) {
  ...
  <p style="margin:0 0 8px;font-size:13px;color:#FA4616;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Don’t miss live alerts</p>
  <p style="margin:0 0 12px;font-size:14px;line-height:1.55;">On your phone: open <strong>My Alerts</strong> → <strong>Save Preferences</strong> (Both + Instant, with Visits / Commits / Scores on) → allow notifications → tap <strong>Send test alert</strong>. That registers this device so verified OV visits, UF commits, and game scores reach you.</p>
...
function checklistBodyHtml({ name, email } = {}) {
  ...
  <p style="margin:0 0 16px;font-size:14px;line-height:1.55;">☑ My Alerts → Save Preferences → Send test alert (so visits, commits, and scores hit your phone)</p>
  ${ctaButton(ALERTS_URL, ALERTS_LINK_LABEL)}
  <p style="margin:16px 0 0;font-size:14px;color:#94a3b8;line-height:1.6;">— GatorVault Media, LLC</p>`;
}
