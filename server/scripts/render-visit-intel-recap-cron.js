...
const RECAP_URL =
  ...
  'https://gatorvault-api.onrender.com/api/futurecast/visit-intel/recap';
...
async function postRecap() {
  ...
  const res = await fetch(RECAP_URL, {
    ...
    headers: {
      ...
      'User-Agent': 'gatorvault-visit-intel-recap-cron/1.0',
    },
    body: '{}',
  ...
  if (!res.ok) {
    const err = new Error(`visit intel recap HTTP ${res.status} (${elapsed}ms)`);
...
(async () => {
  ...
  console.log('[visit-intel-recap-cron] ok', JSON.stringify(result.body));
  process.exit(0);
})().catch((err) => {
  console.error('[visit-intel-recap-cron] failed:', err.message, err.body || '');
  process.exit(0);
});