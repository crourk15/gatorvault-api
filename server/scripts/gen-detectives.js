...
function savePile(doc) {
  ...
  if (doc.cases.length > MAX_CASES) {
    doc.cases = doc.cases.slice(-MAX_CASES);
  }
...
function caseFingerprint(payload = {}) {
  ...
  const key = [
    payload.skipReason || '',
    beat.id || beat.url || '',
    beat.text || cand.text || cand.beatText || '',
    cand.playerSlug || cand.playerName || '',
    payload.skipStage || ''
  ].join('|');
...
function addCase(payload = {}) {
  ...
  const existing = doc.cases.find(
    (c) => c.fingerprint === fp && c.status !== 'resolved' && c.status !== 'expired'
  );
  if (existing) {
...
function countByStatus() {
  const doc = loadPile();
  const out = { pending: 0, investigating: 0, resolved: 0, failed: 0, expired: 0 };
