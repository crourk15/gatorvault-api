...
const DIGEST_URL =
  ...
  'https://gatorvault-api.onrender.com/api/futurecast/visit-intel/daily-digest';
...
async function postDailyDigest() {
  ...
  const res = await fetch(DIGEST_URL, {
    ...
    headers: {
      ...
      'User-Agent': 'gatorvault-visit-intel-daily-digest-cron/1.0',
    },
    body: '{}',
  ...
  if (!res.ok) {
    const err = new Error(`visit intel daily digest HTTP ${res.status} (${elapsed}ms)`);
...
(async () => {
  ...
  console.log('[visit-intel-daily-digest-cron] ok', JSON.stringify(result.body));
  process.exit(0);
})().catch((err) => {
  console.error('[visit-intel-daily-digest-cron] failed:', err.message, err.body || '');
  process.exit(0);
});