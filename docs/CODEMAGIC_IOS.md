...
| Build fails on `Cannot find module 'node-fetch'` | Client seed must not load server YouTube ingest. Use `film-room-gnfp-filter.js` (same pattern as `betting-next-game.js`). |
