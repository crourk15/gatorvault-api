...
- **Hub TEAM/PROGRAM Copy Brief** — still not a FutureCast board packet, but when beat rows carry visit/recruit cues (visitor lists, Swamp Saturdays, star/pos teasers), Open/Copy Brief runs **hub research**: named prospects from stacked beats + nameless cue resolve against Vault (`5-star edge` → board 5★ EDGE with Florida process, e.g. Ghioto). Paste includes a **RESEARCH** block so Cursor never leaves anonymous star teasers. Pure camp/staff with no recruit signal stays thin.
