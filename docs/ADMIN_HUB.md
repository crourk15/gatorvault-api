...
- `POST /api/futurecast/lab-warm` (Admin PIN / cron). Hub-warm cron is **lite** for crash-loop control; `LAB_WARM_URL` on `gatorvault-api-hub-warm` hits lab-warm for 2028 chase/master/trending so Lab plates do not freeze while lite-only runs.
