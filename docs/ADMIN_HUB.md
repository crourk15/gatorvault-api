...
- **2-minute API downs:** the Render disk cannot do zero-downtime. Autodeploy only when `server/**` (except tests/scripts) or `render.yaml` changes — a Netlify edge / client / docs / cron-script commit must not rebuild `gatorvault-api`. After a real API deploy, heavy crons (recruiting-light On3 / beat / allowlist / hub refresh) soft-skip for `BOOT_HEAVY_MIN_UPTIME_SEC` (8 min) so `:00` cannot 502-loop the young instance. Score alerts stay live. `server.js` now listens and answers `/ready` **before** requiring recruiting/admin/auth modules.
...
**Live schedule (no Codemagic for slate + Game Week intel edits):** `GET /api/schedule?year=2026` reads `server/data/schedule/2026-season.json` (Render override `/var/data/schedule/2026-season.json`). Schedule page + Game Week Command Center (Keys / Film Notes / Scouting) fetch that API. Git-bundle keys / film / scout / swing **always** overlay Render durable, even when official-box sync stamps a newer `updatedAt`. Box URLs and official finals still follow the stamp so discovery is not wiped. After the live-fetch client bake is in the iOS binary, add/move games **and** update weekly film intel (`film`, `filmNotes`, `keys`, `opponentTendencies`, `defenseTendencies`, `specialTeams`, `offenseScout`, `defenseScout`, `howUFWins`, `scoutingReport`) by editing the JSON / admin `PUT /api/schedule` only — same pattern as depth chart. `filmNotes` / `opponentTendencies` / `defenseTendencies` / `specialTeams` / `film` = fan Film Notes + Scouting Report. Public `GET /api/schedule` **empties** `offenseScout` / `defenseScout` and **omits** `scoutingReport` so the current App Store mapper falls back to those fan fields (no Codemagic). Raw desk scout stays on disk — `GET /api/schedule?desk=1` + admin PIN, or the `PUT` response. Never dump “film-confirmed” source tags on the main Game Week rail. Each game’s `tickets` object should hold **event deep links** (`official` / Ticketmaster, TickPick, StubHub) — not marketplace search URLs — so Buy Tickets opens that matchup. `GET /api/schedule?year=2026` reads `server/data/schedule/2026-season.json` (Render override `/var/data/schedule/2026-season.json`). Schedule page + Game Week Command Center (Keys / Film Notes / Scouting) fetch that API. After the live-fetch client bake is in the iOS binary, add/move games **and** update weekly film intel (`film`, `filmNotes`, `keys`, `opponentTendencies`, `defenseTendencies`, `specialTeams`, `offenseScout`, `defenseScout`, `howUFWins`, `scoutingReport`) by editing the JSON / admin `PUT /api/schedule` only — same pattern as depth chart. `filmNotes` / `opponentTendencies` / `defenseTendencies` / `specialTeams` / `film` = fan Film Notes + Scouting Report. Public `GET /api/schedule` **empties** `offenseScout` / `defenseScout` and **omits** `scoutingReport` so the current App Store mapper falls back to those fan fields (no Codemagic). Raw desk scout stays on disk — `GET /api/schedule?desk=1` + admin PIN, or the `PUT` response. Never dump “film-confirmed” source tags on the main Game Week rail. Each game’s `tickets` object should hold **event deep links** (`official` / Ticketmaster, TickPick, StubHub) — not marketplace search URLs — so Buy Tickets opens that matchup.

**Game Week scout + keys structure (Ole Miss week forward):** Stamp the pasted scout and 3 keys **verbatim** — full titles and full paragraphs. Do not catchphrase-shorten (`Stay Home` / `Play the Shot` / `Run the Front`) or rewrite the bodies. This week's keys: `Maintain Lane Discipline & Crowd Chambliss` / `Cap the Vertical Shots` / `Establish the Downhill Run Game`. Scouting Report offense (`opponentTendencies`) is schematic identity → efficiency → situational → personnel (QB, WRs, run). Defense (`defenseTendencies`) is schematic identity → pass rush → metrics / red zone → front + names → secondary + the hole. Special teams (`specialTeams`) is kicker truth + 2-pt / 4th-down personality — do not leave the generic “Win field position” defaults on a sat week. Bottom line (`film` / `filmNotes[0]`) is they go as [QB] goes plus the three keys in one sentence. Never Lane Kiffin as 2026 Ole Miss. Golding still calls D. Coverage shells stay off the fan rail.
...
**Periodic / no-Codemagic surfaces:** Footprint Class tabs, hub commits, class overview, home **NOW** pulse (`GET /api/recruiting/hub/ticker`), FutureCast HP / Closing Top UF Targets, allowlist intel, War Room vault evals. Prefer API/data + durable `/var/data/.../hub-runtime` with bundled seed fallback under `server/data/recruiting/hub-runtime/` (and `futurecast-runtime` for Lab HP). Bump `FOOTPRINT_CACHE_REV` when tally logic changes so poisoned runtime disks cannot stick; lite warm rewrites footprint/commits/**ticker** on the ~25m cron. Home NOW is **three weekly pillars**: Game / Visitors (or Road) / Season. ABC lives on Game only. Visitors ticks names. Any Florida commit/flip this week takes the middle slot as **News** — bigger than visitors. On the road (no visitors), that slot fills with the week’s Florida commit if we have one — never NA. Game and Season stay. Optional copy: `server/data/home/weekly-now.json` (`weeks` place/standing + root `seasonTicks` extra Season items — empty the array to drop without Codemagic). `GET /api/recruiting/hub/ticker` is a direct now-pack (`status: ready` + `nowWeek`, no hub-cache `building` — iOS 1.0.29 throws building away). Ticker cache `t16` leftover plates are unused for this GET. Client keeps last-good `nowWeek` and never falls back to the hub-bundle class-rank seed when the 30s refresh misses. Never depends on Codemagic for the weekly copy.
...
- **Vault feed 2028+ (7am / 7pm ET)** — dedicated cron feeds FutureCast Lab + recruiting for **2028/2029/2030+** from trusted beats: updates existing players’ intel, monitor-provisions new names into Vault (API only — no Codemagic), **never auto-adds 2027** (handpick / flip chart), **blocks staff/coach phantoms**. Hourly tick; the script runs at **7–9 AM/PM ET** so an 8:05 / 9:05 catch-up still writes the missed 7am / 7pm slot. The API **accepts immediately** and finishes in the background. Scheduled cron skips allowlist + full beat-ingest (those have their own jobs) so the FutureCast pass can finish on Starter — a dead “running” stamp with no heartbeat is marked error instead of sitting all day. **Run now** refreshes the beat cache first (empty cache used to finish at all zeros). Also merges **On3/Gators Online team-news articles** (title resolve) into Created/Updated — not tweets-only. Proof: FutureCast panel → “Vault feed 2028+” card (created / updated / unresolved / beats fetched / empty reason / allowlist cov) or `GET /api/admin/hub/vault-feed-2028`. Manual: Job Queue `vault-feed-2028-sweep` or Run now on that card. Hard-refresh Hub after deploy (`hub-fc-v8`). Proof tables list Updated (what changed + beat cue) and Unresolved (reason + beat cue).
...
| **Member Activity** | `#members/activity` | Last-seen + short page trail (24h / 7d) · iOS vs website · top pages. Last-seen from sign-in and `/api/session`; page pings from the vault client (iOS trail live since the 1.0.23 bake) |
...
If trials get nothing: (1) check Resend dashboard for delivers/bounces/spam, (2) confirm welcome on a fresh signup returns `emailSent:true` / `provider:resend`, (3) Render cron `gatorvault-api-onboarding-drip` logs (must have `MONITORING_CRON_SECRET` — soft-fail looks green), (4) kill switch `ONBOARDING_DRIP_DISABLED` must be unset. One-shot blasts (iOS update / season preview / **1.0.29 Chase walkthrough**) are separate Admin routes (`POST /api/admin/members/announce-*`) and include active trials. The 1.0.29 letter is **halted** — no boot, `/api/email-status`, or onboarding-cron send. Members → **Send 1.0.29 Chase email** is the only path (stamped `iosAnnounce_1_0_29_chase`; does not re-send unless you pass `force`). Set `IOS_129_CHASE_ANNOUNCE_AUTO=1` only if Charles explicitly wants a boot send again.

Expired locker win-back: `POST /api/admin/members/extend-trial` `{ emails, days: 30, sendEmail: true }` adds 30 days to `trialEnd` (not a paid/manual grant) and emails a come-back note. Members → Expired also has **+30d** per row and **Extend visible expired +30d**. Skips `@gatorvault.test`, tester/demo, App Review, and paid. After Render: Members hub or `node server/scripts/extend-expired-locker-trials.js`.

Company-inbox account deletes: `POST /api/admin/members/delete` `{ confirm: "DELETE", emails }` or `{ confirm: "DELETE", runListed: true }`. Members → **Delete** on a row, or **Delete inbox list** (Cannon / Clawson / McCoy / Wilkerson). No email is sent. Boot also honors that Sep 24 list once (`inbox-account-delete-2026-09-24.json`). Full name or exact email only — last-name-only McCoy is skipped.
