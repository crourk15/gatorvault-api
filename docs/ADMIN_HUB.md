...
- **2-minute API downs:** the Render disk cannot do zero-downtime. Autodeploy only when `server/**` (except tests) or `render.yaml` changes — a Netlify edge / client / docs commit must not rebuild `gatorvault-api`.
