...
**Live schedule (no Codemagic for slate edits):** `GET /api/schedule?year=2026` reads `server/data/schedule/2026-season.json` (Render override `/var/data/schedule/2026-season.json`). Schedule page + Game Week timeline fetch that API. After the live-fetch client bake is in the iOS binary, add/move games by editing the JSON / admin `PUT /api/schedule` only — same pattern as depth chart. Each game’s `tickets` object should hold **event deep links** (`official` / Ticketmaster, TickPick, StubHub) — not marketplace search URLs — so Buy Tickets opens that matchup.
