...
**Remaining-season predictions (weekly, no Codemagic):** After each Saturday, restamp `ufPct` / `pred` / `predUF` / `predOpp` on remaining games from how Florida and those opponents actually looked — not FPI paste, not FCS/G5 blowouts flipping Texas or Georgia. Set board `predThrough` (e.g. `2026-W2`) and bump `updatedAt` so Render overlay heals. Keep `client/lib/schedule-data.ts` in sync (seed fallback only). Auburn week stays the film sit until the Southern Miss box is posted. Rivalry-badge bake is separate — do not wait on Codemagic to move the board.
