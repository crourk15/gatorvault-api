...
Spaced steps **release hub memory** (disk fallback) before bundle/HP, then restore lite. Look for `[recruiting-hub] boot priority-lite warm` / `released hub memory before` / `spaced elite fill queued` / `spaced step` / `warm-memory` in Render logs. Env knobs: `HUB_GET_NO_SYNC_BUILD`, `FC_GET_NO_SYNC_BUILD`, `HUB_BOOT_FORCE_WARM`, `HUB_SPACED_ELITE_WARM`, `HUB_SPACED_WARM_YEARS` (default `2028`), `HUB_SPACED_WARM_GAP_MS` (code floor ≥240s), `HUB_SPACED_WARM_START_MS` (code floor ≥180s), `HUB_SPACED_WARM_MASTER` (opt-in), `HUB_BUNDLE_SEQUENTIAL`, `HUB_SPACED_WARM_FORK` (child-process warm). Stale Render dashboard values below the floors are ignored.
