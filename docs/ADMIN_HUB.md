...
**Game Week scout + keys structure (Ole Miss week forward):** Keys use the full victory-key titles and full football bodies from the scout (`Maintain Lane Discipline & Crowd Chambliss` / `Cap the Vertical Shots` / `Establish the Downhill Run Game`) — not catchphrase shortenings. Scouting Report offense (`opponentTendencies`) is schematic identity → efficiency → situational → personnel (QB, WRs, run). Defense (`defenseTendencies`) is schematic identity → pass rush → metrics / red zone → front + names → secondary + the hole. Special teams (`specialTeams`) is kicker truth + 2-pt / 4th-down personality — do not leave the generic “Win field position” defaults on a sat week. Bottom line (`film` / `filmNotes[0]`) is they go as [QB] goes plus the three keys in one sentence. Never Lane Kiffin as 2026 Ole Miss. Golding still calls D. Coverage shells stay off the fan rail.
