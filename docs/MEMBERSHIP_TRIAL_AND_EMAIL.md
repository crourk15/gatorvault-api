...
- **Server drip** (hourly in-process + Render cron `gatorvault-api-onboarding-drip`):
  ...
  - Day **25** — same letter as 5 days left (`Stay with Gator Nation — {weekday}`). One send stamps both day 25 and `d5` so a 30-day trial cannot get two copies the same hour.
- **Trial-clock convert emails** (based on `trialEnd`, not signup day):
  ...
  - Body: date, home for Gator Nation, membership keeps you inside, app path, Restore. Does not resend to stamped users. In-process hourly drip and the Render cron share a queue lock so they cannot overlap.
