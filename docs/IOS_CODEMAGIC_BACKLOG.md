...
- [x] Home NOW Season extra tick: `GET /api/recruiting/hub/ticker` is a direct now-pack (`status: ready` + `nowWeek`, no hub-cache building). Empty `seasonTicks` to drop extra lines. API after Render — no Codemagic.
...
- [x] Game Week Ole Miss expected visitors: verified the Aug early look against players / visit logs / intel; dropped the `j-c-wessel` duplicate; filled every card with the same pos / year / school line (Craig-James, Lawson, Evans, Winn, Dion, Martenson). Added Vickers + Turner from the later 247/Gators Wire look. Added Domonic Williams Jr. (2028 CB, Jacksonville NC) — first Gainesville trip, player said he will be in the Swamp Ole Miss weekend. Jackson 2029 stayed off — no dated Sep 26 sit. API `/api/schedule` — no Codemagic.
- [x] Vault + allowlist: Domonic Williams Jr. 2028 CB on players / visit log / 2028 board / admin allowlist. Hudl sit now live (Vault Scouting SHOW). API after Render — no Codemagic.
...
- [x] Closest to commit live warm: Home ticker / FutureCast home primes the 2028 HP plate into memory. Current App Store already reads `/api/futurecast/high-priority?year=2028` (no-store). No Codemagic — do not bake a Closest list.
